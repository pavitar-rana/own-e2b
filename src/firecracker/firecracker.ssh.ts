...
import { connectSSH } from "../services/ssh/index.ts";

let connected = false;
let lastErr: any = null;
export const attemptSSHConnection = async ({ maxRetries = 10, vmIP, baseDelay = 1000 }) => {
    console.log(`\n Waiting for SSH to become available at ${vmIP}:22...`);
    console.log(`Max retries: ${maxRetries}, Base delay: ${baseDelay}ms\n`);

    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        console.log(`[${attempt}/${maxRetries}] Trying SSH connection to ${vmIP}...`);
        try {
            ...
            lastErr = err;
            const backoff = baseDelay * Math.min(attempt, 5);
